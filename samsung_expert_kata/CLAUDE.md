...
풀이마다 다시 유도하지 않도록 정리해 둔다. UsedCarManager 에서 도출됨.

### 호출 빈도의 비대칭을 먼저 센다

자료구조를 고르기 전에 **각 진입점이 몇 번 불리는지부터 센다.**
UsedCarManager 는 케이스당 `buy` 1,000,000 / `sell` 10,000 / `refund` 100 이었다. 100 배 차이다.

> **가장 자주 불리는 연산에는 아무것도 얹지 않는다.
> 그 대가로 드물게 불리는 연산이 일을 더 하게 만든다.**

검색을 빠르게 하려고 트리를 얹는 설계는 100 배 자주 불리는 `buy` 에 비용을 떠넘기는 것이라
정반대 방향이었다. 버킷 배열에 뒤에 붙이기만 하는 O(1) `buy` 가 정답이었다.
...
UsedCarManager 실측 (재고 약 30,000 대):

| engine 분할 | 탐색 버킷 | 훑는 원소 | 합계 |
|---|---|---|---|
| 안 나눔 | 24 | 3,264 | 3,288 |
| 32 칸 | 256 | 약 1,300 | **약 1,550** |
| 4000 칸 | 32,000 | 약 1,100 | 33,100 |

4000 칸은 버킷 88 만 개에 차 3 만 대가 흩어져 대부분이 빈 서랍이 된다.
...
(UsedCarManager: 버킷당 최대 154 → 200, 한 주문 최대 6,054 대 → 10,000)

### 부분 최적화의 효과는 그 부분의 비중까지다

`sell` 을 2 배 빠르게 해도 전체가 2 배 빨라지지 않는다.
`buy` 가 쓰는 시간은 그대로 남기 때문이다.
(모델은 2.1 배를 예상했으나 실제는 1.6 배였다)
